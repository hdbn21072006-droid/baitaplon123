              ...
              <Space size={16} style={{ marginBottom: 8 }}>
                {detail.cccd_front_url && (
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <Image src={`${process.env.UMI_APP_API_URL || 'http://localhost:5000'}${detail.cccd_front_url}`} width={180} height={110} style={{ objectFit: 'cover', borderRadius: 6 }} fallback="data:image/png;base64,iVBORw0KGgo=" />
                  </div>
                )}
                {detail.cccd_back_url && (
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ marginBottom: 4 }}><Text type="secondary">CCCD mặt sau</Text></div>
                    <Image src={`${process.env.UMI_APP_API_URL || 'http://localhost:5000'}${detail.cccd_back_url}`} width={180} height={110} style={{ objectFit: 'cover', borderRadius: 6 }} fallback="data:image/png;base64,iVBORw0KGgo=" />
                  </div>
                )}
                {detail.avatar_url && (
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ marginBottom: 4 }}><Text type="secondary">Ảnh chân dung</Text></div>
                    <Image src={`${process.env.UMI_APP_API_URL || 'http://localhost:5000'}${detail.avatar_url}`} width={100} height={130} style={{ objectFit: 'cover', borderRadius: 6 }} fallback="data:image/png;base64,iVBORw0KGgo=" />
                  </div>
                )}
              </Space>
            </>
